            ...
            function(result)
            {
              ...
              db.upsert('stats', {rlrank_id: data.rlrank_id, type: data.type}, data,
                function(err, doc)
                {
                  if (err)
                  {
                    console.log('[STATS] Could not save player rank to "ranks" DB', rank, err); // ERROR
                  }
                }
              );
              ...
              db.insert('ranksHistorical', data,
                function(err, doc)
                {
                  if (err)
                  {
                    console.log('[STATS] Could not save player rank to "ranksHistorical" DB', rank, err); // ERROR
                  }
                }
              );
            }
